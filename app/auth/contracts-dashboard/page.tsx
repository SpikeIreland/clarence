...
export default function ContractsDashboard() {
  ...
  // WP6: Inline contract name editing
  ...
  // ==========================================================================
  // SECTION 9B: INLINE CONTRACT NAME EDIT
  // ==========================================================================

  const startEditingName = (session: Session) => {
    setEditingSessionId(session.sessionId)
    setEditingName(session.contractName || session.sessionNumber || '')
  }
  ...
  const saveContractName = async (sessionId: string) => {
    if (!editingName.trim()) {
      cancelEditingName()
      return
    }
    ...
    try {
      const { error } = await supabase
        .from('sessions')
        .update({ contract_name: editingName.trim() })
        .eq('session_id', sessionId)

      if (error) {
        console.error('Error updating contract name:', error)
        alert('Failed to update contract name. Please try again.')
      } else {
        // Update local state
        setSessions(prev => prev.map(s =>
          s.sessionId === sessionId
            ? { ...s, contractName: editingName.trim() }
            : s
        ))
      }
    } catch (error) {
      console.error('Error saving contract name:', error)
      alert('Failed to update contract name. Please try again.')
    } finally {
  ...
  // ==========================================================================
  // SECTION 12B: DASHBOARD CHAT FUNCTIONS
  // ==========================================================================
  ...
  async function sendChatMessage() {
    ...
    try {
      const dashboardContext = {
        ...
        activeContracts: sessions.filter(s => s.status !== 'completed' && !s.isTraining).length,
        completedContracts: sessions.filter(s => s.status === 'completed').length,
        totalValue: sessions.reduce((sum, s) => {
          const mapped = dealValueMap[s.dealValue] || parseInt(s.dealValue || '0') || 0
          return sum + mapped
        }, 0),
  ...
  useEffect(() => {
    if (showChatOverlay && chatMessages.length === 0) {
      const welcomeMessage: ChatMessage = {
        id: '1',
        type: 'clarence',
        content: `Hello ${userInfo?.firstName || 'there'}! I'm CLARENCE, your contract negotiation assistant. I can help you understand your contracts, explain the negotiation process, or answer general legal and commercial questions. What would you like to explore today?`,
        timestamp: new Date()
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex items-center gap-4">
                            <div>
                                ...
                                {editingSessionId === session.sessionId ? (
                                  ...
                                  <div className="flex items-center gap-2">
                                    <input
                                      ...
                                      onKeyDown={(e) => handleNameKeyDown(e, session.sessionId)}
                                      onBlur={() => saveContractName(session.sessionId)}
                                      autoFocus
                                      disabled={isSavingName}
                                      className="text-lg font-semibold text-slate-800 bg-white border border-slate-300 rounded px-2 py-0.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                              ...
                              {session.contractName && session.sessionNumber && editingSessionId !== session.sessionId && (
      ...
      {/* ================================================================== */}
      {/* SECTION 31: FLOATING CHAT BUTTON */}
      {/* ================================================================== */}
      {!showChatOverlay && (
        <button
          onClick={() => setShowChatOverlay(true)}
          className="fixed bottom-6 right-6 bg-emerald-600 hover:bg-emerald-700 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all z-40 group"
          title="Chat with CLARENCE"
        >
          ...
          {/* Tooltip */}
          <span className="absolute bottom-full right-0 mb-2 px-3 py-1.5 bg-slate-800 text-white text-xs rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
