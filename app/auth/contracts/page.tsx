...
export default function ContractLibraryPage() {
    ...
    const loadTemplates = useCallback(async (user: UserInfo) => {
        try {
            ...
            const companyTemplates = allTemplates.filter(t =>
                !t.isSystem &&
                !t.sourceSessionId &&
                (t.companyId === user.companyId || t.companyId == null)
            )
    ...
    // ==========================================================================
    // SECTION 7A: DELETE TEMPLATE HANDLER
    // ==========================================================================
    ...
    const handleDeleteTemplate = async (templateId: string) => {
        ...
        try {
            ...
            // Step 2: Delete the template (only if user owns it)
            const { error: templateError } = await supabase
                .from('contract_templates')
                .delete()
                .eq('template_id', templateId)
                .eq('created_by_user_id', userInfo.userId)  // Safety: only delete own templates
                .eq('is_system', false)  // Safety: never delete system templates
                .eq('is_public', false)  // Safety: never delete company templates from here
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-slate-800">Delete Template?</h3>
                                <p className="text-sm text-slate-500">This action cannot be undone.</p>
                            </div>
                        </div>

                        <p className="text-slate-600 mb-6">
                            Are you sure you want to permanently delete this template and all its clauses?
                        </p>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-2.5 px-4 text-sm font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors disabled:opacity-50"
                            >
                                Cancel
                            </button>
                            <button
