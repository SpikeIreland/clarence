...
// ========== SECTION 5: USE EFFECTS ==========
  ...
  useEffect(() => {
    ...
    }, [router, searchParams]) // Dependencies intentionally limited to prevent loops
  ...
  return (
    ...
    <div className="flex flex-1 overflow-hidden">
        {/* Sidebar - Fixed width constraint */}
        <div className={`bg-slate-800 text-white transition-all duration-300 flex-shrink-0 ${
            ...
            <div className="p-4 overflow-y-auto h-full">
              ...
              {sessions.length > 0 ? (
                <div className="space-y-2">
                  {Object.values(
                    sessions.reduce((acc: Record<string, Session & { providers: Provider[] }>, session) => {
                  ...
                  ).map((session: Session & { providers: Provider[] }) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-sm truncate">{session.customerCompany}</div>
                            <div className="text-xs text-slate-400 truncate">{session.sessionNumber || session.sessionId.substring(0, 13)}</div>
                            <div className="text-xs text-slate-300 mt-1 truncate">{session.serviceRequired}</div>
                          </div>
                          <svg 
                        ...
                        {session.providers && session.providers.length > 0 ? (
                          session.providers.map((provider: Provider) => (
                            <div
                              ...
                              className={`p-2 mb-1 ml-2 mr-2 rounded cursor-pointer text-sm transition ${
                            ...
                            >
                              <div className="flex justify-between items-center">
                                ...
                                <span className="text-xs bg-slate-900 px-2 py-1 rounded flex-shrink-0 ml-2">
                                  {provider.score || 80}%
                                </span>
                              </div>
                            </div>
