        ...
        return (
            <div className="h-full flex items-center justify-center">
                <div className="text-center max-w-md">
                    ...
                    <h3 className="text-xl font-semibold text-slate-700 mb-3">Select a Document</h3>
                    <p className="text-slate-500">
                        Choose a document from the left panel to preview its contents and generate or download it.
                    </p>
                </div>
            </div>
        )
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex-shrink-0 p-6 bg-white border-b border-slate-200">
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-2">
                        {document.status === 'ready' || document.status === 'final' ? (
                            <>
                                <button
                                    onClick={() => onDownload(document.id, 'pdf')}
                                    className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium transition flex items-center gap-2"
                                >
                                    <span>📕</span> PDF
                                </button>
                                <button
                                    onClick={() => onDownload(document.id, 'docx')}
                                    className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg font-medium transition flex items-center gap-2"
                                >
                                    <span>📘</span> DOCX
                                </button>
                            </>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
                {document.status === 'locked' ? (
                    <LockedDocumentView document={document} />
                ) : document.status === 'generating' ? (
                    <GeneratingDocumentView document={document} />
                ) : document.status === 'ready' || document.status === 'final' || document.status === 'in_progress' ? (
                    <DocumentContentPreview document={document} session={session} />
                ) : (
                    <EmptyDocumentView document={document} onGenerate={() => onGenerate(document.id)} />
                )}
            </div>
        </div>
    )
...
function LockedDocumentView({ document }: { document: DocumentItem }) {
    return (
        <div className="h-full flex items-center justify-center">
            <div className="text-center max-w-md">
                ...
                <h3 className="text-lg font-semibold text-slate-700 mb-2">Document Locked</h3>
                <p className="text-slate-500 mb-4">
                    This document requires other documents to be completed first.
                </p>
                {document.prerequisites.length > 0 && (
                    <div className="bg-white rounded-lg p-4 border border-slate-200">
                        <p className="text-sm text-slate-600 mb-2">Prerequisites:</p>
...
function GeneratingDocumentView({ document }: { document: DocumentItem }) {
    return (
        <div className="h-full flex items-center justify-center">
            <div className="text-center max-w-md">
                ...
                {(document.progress ?? 0) > 0 && (
                    <div className="w-full max-w-xs mx-auto">
                        ...
                        <p className="text-xs text-slate-400 mt-2">{document.progress}% complete</p>
                    </div>
                )}
            </div>
        </div>
...
function EmptyDocumentView({ document, onGenerate }: { document: DocumentItem; onGenerate: () => void }) {
    return (
        <div className="h-full flex items-center justify-center">
            <div className="text-center max-w-md">
...
function DocumentContentPreview({ document, session }: { document: DocumentItem; session: Session | null }) {
    // This would show a preview of the generated document
    // For now, we'll show a placeholder based on document type
    ...
    return (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
                {renderDocumentPreviewContent(document, session)}
            </div>
        </div>
    )
}

function renderDocumentPreviewContent(document: DocumentItem, session: Session | null) {
    switch (document.id) {
        case 'executive-summary':
            return (
                <div className="prose prose-sm max-w-none">
                    <div className="text-center mb-6">
                        <h1 className="text-2xl font-bold text-slate-800 mb-2">NEGOTIATION EXECUTIVE SUMMARY</h1>
                        <p className="text-slate-500">Session: {session?.sessionNumber || 'Loading...'}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        <div className="bg-emerald-50 rounded-lg p-4">
                            <h3 className="font-semibold text-emerald-800 mb-2">Customer</h3>
                            <p className="text-emerald-700">{session?.customerCompany || '—'}</p>
                        </div>
                        <div className="bg-blue-50 rounded-lg p-4">
                            <h3 className="font-semibold text-blue-800 mb-2">Provider</h3>
                            <p className="text-blue-700">{session?.providerCompany || '—'}</p>
                        </div>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-4 mb-4">
                        <h3 className="font-semibold text-slate-700 mb-2">Deal Overview</h3>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <span className="text-slate-500">Contract Type:</span>
                                <span className="ml-2 text-slate-700">{session?.serviceType || '—'}</span>
                            </div>
                            <div>
                                <span className="text-slate-500">Deal Value:</span>
                                <span className="ml-2 text-slate-700">{session?.dealValue || '—'}</span>
                            </div>
                            <div>
                                <span className="text-slate-500">Alignment:</span>
                                <span className="ml-2 text-emerald-600 font-semibold">{session?.alignmentPercentage || 0}%</span>
                            </div>
                            <div>
                                <span className="text-slate-500">Phase:</span>
                                <span className="ml-2 text-slate-700">{session?.phase || 0} of 6</span>
                            </div>
                        </div>
                    </div>
        ...
        case 'contract-handbook':
            return (
                <div className="prose prose-sm max-w-none">
                    <div className="text-center mb-6">
                        <h1 className="text-2xl font-bold text-slate-800 mb-2">📘 CONTRACT HANDBOOK</h1>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                {messages.length === 0 && (
                    <div className="text-center py-8">
                        ...
                        <h3 className="font-semibold text-slate-700 mb-2">How can I help?</h3>
                        <p className="text-sm text-slate-500 max-w-xs mx-auto">
                            Ask me about any document, or request help generating specific content.
                        </p>
                    </div>
                )}
...
function DocumentCentreContent() {
    ...
    const initializeDocuments = useCallback((session: Session): DocumentItem[] => {
        // Initialize documents with status based on session state
        return DOCUMENT_DEFINITIONS.map(def => {
            let status: DocumentStatus = 'locked'
            let progress = 0

            // Determine initial status based on prerequisites and session state
            const prerequisitesMet = def.prerequisites.every(prereqId => {
                // For now, assume no prerequisites are met initially
                return false
            })

            if (def.prerequisites.length === 0) {
                // No prerequisites - can generate immediately
                status = 'in_progress'
                progress = 0
            ...
            // Mock: If alignment is high, some docs might be ready
