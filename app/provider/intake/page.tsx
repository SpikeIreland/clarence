...
function ProviderIntakeContent() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.ok) {
                // Navigate to questionnaire (next step in provider journey)
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
                <div className="bg-slate-800/50 backdrop-blur rounded-2xl border border-slate-700/50 p-8 max-w-md text-center">
                    ...
                    <h2 className="text-xl font-medium text-white mb-2">Invalid Invitation</h2>
                    <p className="text-slate-400 mb-6">{errorMessage || 'This invitation link is not valid or has expired.'}</p>
                    <Link
                        href="/provider"
                        className="inline-block px-6 py-2.5 bg-slate-700 text-white rounded-lg hover:bg-slate-600 transition"
                    >
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
                    ...
                    <div className="flex justify-between h-16">
                        <div className="flex items-center">
                            <div>
                                ...
                                <div className="text-xs text-slate-500 tracking-widest font-light">PROVIDER INTAKE</div>
                            </div>
                        </div>
                        <div className="flex items-center">
                            <span className="text-slate-400 text-sm">{formData.contactEmail}</span>
                        </div>
                    </div>
                </div>
            </nav>
            ...
            {/* Opportunity Banner */}
            <div className="bg-gradient-to-r from-emerald-600/20 to-blue-600/20 border-b border-emerald-500/30 py-4">
                <div className="max-w-5xl mx-auto px-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-8">
                            <div>
                                <span className="text-emerald-400/70 text-xs">Opportunity</span>
                                <div className="text-white font-medium">{inviteData?.serviceRequired || 'Service Contract'}</div>
                            </div>
                            <div>
                                <span className="text-emerald-400/70 text-xs">Customer</span>
                                <div className="text-white font-medium">{inviteData?.customerCompany || '—'}</div>
                            </div>
                            <div>
                                <span className="text-emerald-400/70 text-xs">Est. Value</span>
            ...
            <div className="max-w-3xl mx-auto px-4 py-8">
                ...
                <div className="mb-8">
                    <div className="flex justify-between mb-2">
                        <span className="text-sm text-slate-400">Step {currentStep} of {totalSteps}: {getStepName(currentStep)}</span>
                ...
                <div className="mt-6 flex justify-between">
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-lg transition-all cursor-pointer ${currentStep === 1
                    ...
                    {currentStep < totalSteps ? (
                        <button
                            onClick={nextStep}
                            className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-500 transition-all cursor-pointer"
                        >
                            Next →
                        </button>
                    ) : (
                        <button
                            ...
                            disabled={submitting}
                            className="px-8 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-lg hover:from-emerald-500 hover:to-emerald-400 disabled:from-slate-600 disabled:to-slate-500 disabled:cursor-not-allowed transition-all flex items-center gap-2 cursor-pointer"
                        >
                            {submitting ? (
                                <>
...
function CompanyInfoStep({ formData, updateFormData }: StepProps) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Company Information</h2>
            <p className="text-slate-400 text-sm mb-6">Tell us about your organization</p>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        type="text"
                        className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white placeholder-slate-500"
                ...
                <div>
                    ...
                    <select
                        className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white"
...
function ServiceCapabilitiesStep({ formData, toggleArrayItem }: ArrayStepProps) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Service Capabilities</h2>
            <p className="text-slate-400 text-sm mb-6">Select all services you can provide</p>

            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">
                    Primary Services <span className="text-red-400">*</span>
                </label>
                <div className="grid grid-cols-2 gap-2">
                    {PRIMARY_SERVICE_OPTIONS.map(service => (
                        <label key={service} className="flex items-center gap-2 p-3 bg-slate-700/30 rounded-lg cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                            <input
                                type="checkbox"
                                checked={(formData.primaryServices || []).includes(service)}
                                onChange={() => toggleArrayItem('primaryServices', service)}
                                className="rounded border-slate-500 text-emerald-500 focus:ring-emerald-500 bg-slate-600"
                            />
            ...
            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Secondary Services</label>
                <div className="grid grid-cols-2 gap-2">
            ...
            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Industry Specializations</label>
                <div className="grid grid-cols-2 gap-2">
                    {INDUSTRY_OPTIONS.map(industry => (
                        <label key={industry} className="flex items-center gap-2 p-3 bg-slate-700/30 rounded-lg cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                            <input
                                type="checkbox"
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Geographic Coverage</label>
                    <div className="space-y-2">
                        {GEOGRAPHY_OPTIONS.map(geo => (
                            <label key={geo} className="flex items-center gap-2 p-2 bg-slate-700/30 rounded cursor-pointer hover:bg-slate-700/50 transition">
                                <input
                                    type="checkbox"
                                    ...
                                    className="rounded border-slate-500 text-emerald-500 focus:ring-emerald-500 bg-slate-600"
                                />
                                <span className="text-sm text-slate-300">{geo}</span>
                            </label>
                        ))}
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Language Capabilities</label>
                    <div className="space-y-2">
                        {LANGUAGE_OPTIONS.map(lang => (
                            <label key={lang} className="flex items-center gap-2 p-2 bg-slate-700/30 rounded cursor-pointer hover:bg-slate-700/50 transition">
                                <input
                                    type="checkbox"
...
function CertificationsStep({ formData, updateFormData }: StepProps) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Certifications & Compliance</h2>
            <p className="text-slate-400 text-sm mb-6">Select all certifications your organization holds</p>

            <div className="grid grid-cols-2 gap-4">
                <label className="flex items-center gap-3 p-4 bg-slate-700/30 rounded-xl cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                    <input
                        type="checkbox"
                        checked={formData.iso27001 || false}
                        onChange={(e) => updateFormData('iso27001', e.target.checked)}
                        className="rounded border-slate-500 text-emerald-500 focus:ring-emerald-500 bg-slate-600 w-5 h-5"
                    />
                    <div>
                        <span className="text-white font-medium">ISO 27001</span>
                        <p className="text-xs text-slate-400">Information Security Management</p>
                    </div>
                </label>
                ...
                <label className="flex items-center gap-3 p-4 bg-slate-700/30 rounded-xl cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Service Organization Control</p>
                    </div>
                </label>
                ...
                <label className="flex items-center gap-3 p-4 bg-slate-700/30 rounded-xl cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Data Protection Regulation</p>
                    </div>
                </label>
                ...
                <label className="flex items-center gap-3 p-4 bg-slate-700/30 rounded-xl cursor-pointer hover:bg-slate-700/50 transition border border-slate-600/30">
                    ...
                    />
                    <div>
                        <span className="text-white font-medium">Cyber Essentials Plus</span>
                        <p className="text-xs text-slate-400">UK Government Cybersecurity</p>
                    </div>
                </label>
            </div>

            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Key Differentiators</label>
                <textarea
                    className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white placeholder-slate-500"
                    rows={4}
            ...
            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Case Study / Reference</label>
                <textarea
                    className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white placeholder-slate-500"
                    rows={3}
...
// ============================================================================
// STEP 4: Commercial Profile
// ============================================================================

function CommercialProfileStep({ formData, updateFormData }: StepProps) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Commercial Profile</h2>
            <p className="text-slate-400 text-sm mb-6">Define your commercial terms and pricing</p>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Minimum Project Value</label>
                    <select
                        className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white"
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Preferred Contract Duration</label>
                    <select
                        className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white"
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-300 mb-1">Pricing Flexibility</label>
                    <select
                        className="w-full px-4 py-2.5 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white"
            ...
            <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Provider Leverage Position</label>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Market Position</label>
                        <select
                            className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white text-sm"
                    ...
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Pipeline Strength</label>
                        <select
                            className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white text-sm"
                    ...
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Alternative Clients</label>
                        <select
                            className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white text-sm"
                    ...
                    <div>
                        <label className="block text-xs text-slate-400 mb-1">Strategic Interest</label>
                        <select
                            className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-white text-sm"
...
function ContractPositionsStep({ formData, updateFormData }: StepProps) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Contract Positions</h2>
            <p className="text-slate-400 text-sm mb-6">Set your starting positions on key contract terms</p>
...
function ReviewStep({ formData, inviteData }: { formData: Partial<ProviderCapabilities>, inviteData: InviteData | null }) {
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-light text-white mb-2">Review Your Submission</h2>
            <p className="text-slate-400 text-sm mb-6">Please review your information before continuing</p>

            <div className="space-y-4">
                <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                    <h3 className="font-medium text-emerald-400 mb-2 text-sm uppercase tracking-wide">Company</h3>
                    <p className="text-white font-medium">{formData.companyName}</p>
                    <p className="text-sm text-slate-400">{formData.contactName} • {formData.contactEmail}</p>
                    <p className="text-sm text-slate-400">{formData.companySize} • {formData.yearsInBusiness} years • {formData.annualRevenue}</p>
                </div>

                <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                    <h3 className="font-medium text-emerald-400 mb-2 text-sm uppercase tracking-wide">Services</h3>
                    <p className="text-white">{(formData.primaryServices || []).join(', ') || 'Not specified'}</p>
                ...
                <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                    <h3 className="font-medium text-emerald-400 mb-2 text-sm uppercase tracking-wide">Commercial</h3>
                    <p className="text-white">£{formData.dailyRateMin} - £{formData.dailyRateMax} / day</p>
                    <p className="text-sm text-slate-400">
                ...
                <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                    <h3 className="font-medium text-emerald-400 mb-2 text-sm uppercase tracking-wide">Contract Positions</h3>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                        <p className="text-slate-400">Liability Cap: <span className="text-white">{formData.liabilityCap}%</span></p>
                        <p className="text-slate-400">Payment Terms: <span className="text-white">{formData.paymentTermsDays} days</span></p>
                        <p className="text-slate-400">SLA: <span className="text-white">{formData.slaCommitment}%</span></p>
                        <p className="text-slate-400">Termination: <span className="text-white">{formData.terminationNotice} days</span></p>
                    </div>
                </div>

                <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600/30">
                    <h3 className="font-medium text-emerald-400 mb-2 text-sm uppercase tracking-wide">Certifications</h3>
                    <div className="flex flex-wrap gap-2">
                        {formData.iso27001 && <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs">ISO 27001</span>}
                        {formData.soc2TypeII && <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs">SOC 2 Type II</span>}
                        {formData.gdprCompliant && <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs">GDPR</span>}
                        {formData.cyberEssentialsPlus && <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs">Cyber Essentials+</span>}
                    </div>
                </div>
            </div>
